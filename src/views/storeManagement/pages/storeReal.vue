<template>
  <div class="page-main plr10">
    ...
    <div class="mainContent">
      <div class="tableWrap table-maxheight">
        <el-table ref="table" v-loading="loading" :data="tableData" height="100%" border :class="searchClass">
          <el-table-column prop="accountCode" min-width="120" label="店铺代号" align="center" />
          <el-table-column label="平台" min-width="130" align="center">
            <template slot-scope="{}">
              <img :src="platformImg" height="40px">
            </template>
          </el-table-column>
          <el-table-column prop="account" min-width="200" label="账户名" align="center" />
    ...
    <!-- 修改账号密码 -->
    <realSetting :model-visible.sync="settingVisible" :module-data="rowInfo" :grant-status="grantStatus"
      @search="fetch" />
  </div>
</template>
...
import realSetting from "./components/realSetting";
export default {
  name: "StoreManagementStoreReal",
  components: { realSetting },
  mixins: [FETCH],
  data() {
    return {
      platformImg: require('@/assets/images/real.png'),
      searchOptions: {
        enable: 1,
        authStatusList: [],
        accountCodeList: [],
  ...
  methods: {
    init() {
      this.mapi = this.api.storeManagement.real.list;
      this.apiType = "post";
    },
    marketSetting(row) {
