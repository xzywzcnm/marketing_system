<template>
  <div class="dialog-content">
    <el-dialog :visible.sync="dialogVisible" width="80%" :close-on-click-modal="false" class="ebayReplenishmenPage">
      <div slot="title" class="dialog-title">
        <span class="titles">库存管理</span>
        <span class="itemids">itemId: {{ data.itemId }}</span>
      </div>
      <div class="mainContent" style="height:600px;">
        <el-form ref="ruleForm" :model="ruleForm" class="tableWrap formb0" style="height:100%;">
          <el-table ref="table" v-loading="loading" :data="ruleForm.tableData" height="100%" border
            ...
            <el-table-column label="平台SKU" prop="platformSku" align="center" min-width="120px" />
            ...
            <el-table-column label="商品状态" prop="sku" align="center" width="100px">
              <template slot-scope="{row}">
                <!-- <span v-for="gitem in goodsStatusArr" :key="'goodsStatus' + gitem.value">
                  {{ gitem.value === row.goodsStatus ? gitem.label : '' }}
                </span> -->
            ...
            <el-table-column label="库存" width="120px" align="center" fixed="right">
              <template slot-scope="{row,$index}">
            ...
            <el-table-column label="补货保持值" width="120px" align="center" fixed="right">
              <!-- 停售状态变更为不允许开启自动补货 -->
            ...
            <el-table-column label="超卖警戒值" width="120px" align="center" fixed="right">
              <!-- 除了无状态/盲盒都可开启防超卖 -->
              <template v-if="!(row.skuStatusList.includes(6) || row.skuStatusList.includes(8))"
                slot-scope="{row,$index}">
                <el-form-item :prop="'tableData.' + $index + '.syncInventoryWarningValue'"
                  :rules="[{ validator: validOverSaleNum, trigger: 'blur' }]">
                  <drop-input v-model.number="row.syncInventoryWarningValue" :disabled="!row.invenChecked"
                    :list="dropdownList" :index="$index" type="number" @dropdownCommand="overSaleCommand" />
                </el-form-item>
              </template>
            </el-table-column>
            <el-table-column align="center" width="100px" fixed="right">
            ...
            <el-table-column align="center" width="130px" fixed="right">
              <template slot="header" slot-scope="{}">
                <span class="flexAllcenter">
                  <span class="mr10">
...
export default {
  ...
  data() {
    return {
      ...
      // goodsStatusArr: getSalesStatus(['1', '2', '3', '4', '5', '6', '8']),//ERP商品状态
  ...
  methods: {
    ...
    getDetail() {
      ...
      let plantformData = {
        'ebay': {
          api: _self.api.ebay.ebayQueryErpSkuDetailByLink,
          data() {
          ...
          resultFun(data) {
            ...
            _self.ruleForm.tableData = data.map(k => {
              ...
              // 处理仓库
              let userWarehouseList = _self.userWarehouseList;
              k.warehouseIdList = k.erpWarehouseVOList.filter(fk => {
                let flag = false;
                userWarehouseList.forEach(userWare => {
                  !flag && userWare.id === fk.id && (flag = true);
                })
                return flag;
              }).map(ware => ware.id);
    ...
    confirmHandle() {
      this.$refs['ruleForm'].validate((valid) => {
        ...
        this.ruleForm.tableData.forEach((k, i) => {
          ...
          let syncInventoryWarningValue = Number(k.syncInventoryWarningValue) === Number(dk.syncInventoryWarningValue);//警戒值
          ...
          let warehouseIdList = JSON.stringify(k.warehouseIdList) === JSON.stringify(dk.warehouseIdList);//仓库

          if (replenish && autoChecked && invenChecked && ignoreChecked && inventory && warehouseIdList && syncInventoryWarningValue) return;
          list.push(k);
        })
        if (!list.length) {
        ...
        let plantformData = {
          'ebay': {
            api: _self.api.ebay.ebayUpdateAutoReplenishment,
            data: () => {
              return list.map(k => {
                return {
                  linkDetailId: k.linkDetailId,
                  autoReplenishment: k.autoChecked ? k.autoReplenishment : 0,
                  ignoreInitialization: k.ignoreChecked ? 1 : 0,
                  newInventoryQuantity: k.inventoryQuantity || 0,
                  syncInventoryStatus: k.invenChecked ? 1 : 0,
                  syncInventoryWarningValue: k.syncInventoryWarningValue,
                  warehouseIdList: k.warehouseIdList,
                }
              })
            },
          },
        ...
        replenishment() {
          ...
          _self.ruleForm.tableData.forEach((k, i) => {
            ...
            if (checked) {
              ...
              _self.$set(_self.ruleForm.tableData[i], 'syncInventoryWarningValue', '');
            } else {
              _self.$set(_self.ruleForm.tableData[i], 'autoReplenishment', '');
            }
          })
        ...
        inventory() {
          ...
          _self.ruleForm.tableData.forEach((k, i) => {
            ...
            if (checked) {
              ...
              _self.$set(_self.ruleForm.tableData[i], 'autoReplenishment', '');
              // 停售状态的超卖警戒值固定为0，不可以更改
              k.skuStatusList.includes(0) && _self.$set(_self.ruleForm.tableData[i], 'syncInventoryWarningValue', 0);
            } else {
              _self.$set(_self.ruleForm.tableData[i], 'syncInventoryWarningValue', '');
            }
          })
      ...
      let temp = {
        ...
        replenishment() {
          if (row.autoChecked) {
            ...
            _self.$set(_self.ruleForm.tableData[index], 'syncInventoryWarningValue', '');
          } else {
            _self.$set(_self.ruleForm.tableData[index], 'autoReplenishment', '');
          }
        ...
        inventory() {
          if (row.invenChecked) {
            ...
            _self.$set(_self.ruleForm.tableData[index], 'autoReplenishment', '');
            // 停售状态的超卖警戒值固定为0，不可以更改
            row.skuStatusList.includes(0) && _self.$set(_self.ruleForm.tableData[index], 'syncInventoryWarningValue', 0);
          } else {
            _self.$set(_self.ruleForm.tableData[index], 'syncInventoryWarningValue', '');
          }
          temp.autoAndInventory();
    ...
    // 防超卖下拉返回值
    overSaleCommand(data) {
      if (data.command != 1) return;
      let tableData = this.ruleForm.tableData || [];
      let num = tableData[data.index].syncInventoryWarningValue;
      tableData.forEach((row, i) => {
        if (!row.invenChecked) return;
        this.$set(this.ruleForm.tableData[i], 'syncInventoryWarningValue', num);
      })
    },
    // 防超卖输入限制
